...
# When the trigger occurred, an array of 14,000,000 measurement points (bytes)
# is transferred to the host computer and visualized using matplotlib.
# Large values of the timescale parameter (e.g. 1.0, 2.0, 5.0 sec) were tested.
############################################################################
...
print ('RUN...')
cmdWrite(':RUN')
cmdWrite(':ACQ:MDEP {:d}'.format(MEM_DEPTH) )
time.sleep(1.0)
...
sampling_rate = float(cmdRead(':ACQ:SRAT?',0.5).strip())
sampling_rate_khz = (1e-3) * sampling_rate

print ( 'Memory Depth : %s' % '{:,}'.format( mem_depth ) )
print ( 'Sampling Rate: %.3f ksps'  % sampling_rate_khz )

data = b''
read_count = 7
read_bytes = (mem_depth/read_count)
start_pos  = 0
end_pos    = read_bytes

for i in range(read_count):
    print (start_pos+1, end_pos)
    cmdWrite(':WAV:STAR %d' % (start_pos+1) )        # start address
    cmdWrite(':WAV:STOP %d' % end_pos )        # stop address
...
if time_per_div > 0.1:
    start_index = int(0.14*timescale/x_inc) 
    data = data[start_index:]

data_len = len(data)
t_left   = (xref + xorg)
t_right  = (t_left + (data_len)*x_inc )
#t_right  = (t_left + (data_len)/sampling_rate )
...
plot.plot(ts, data)
plot.title( 'Waveform Capture [CH1; {:,} Points; {:,} Ksps]'.format(data_len, int(sampling_rate_khz)) )
